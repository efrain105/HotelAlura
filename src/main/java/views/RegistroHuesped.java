...
import static views.ReservasView.*;
...
public class RegistroHuesped extends JFrame {
    ...
    private RepositorioHuesped huespedRepo = new RepositorioHuesped(em);
    private RepositorioReserva reservaRepo = new RepositorioReserva(em);
    ...
    public RegistroHuesped() {
        ...
        //No necesito asignar manualmente numnero de reserva, porque esta onetoone y se hace automaticamente
        ...
        btnguardar.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                try {
                    ...
                    huesped.setTelefono(new BigDecimal(txtTelefono.getText()));
                    huesped.setNacionalidad((String) txtNacionalidad.getSelectedItem());
                    Date fechaNancorrecta = null;
                    if (txtFechaN.getDate() != null) {
                        Date date = new Date();
                        LocalDate fechaNDate = txtFechaN.getDate().toInstant().atZone(ZoneId.systemDefault()).toLocalDate();
                        LocalDate date1 = date.toInstant().atZone(ZoneId.systemDefault()).toLocalDate();
                        long diferenciaDias = ChronoUnit.YEARS.between(fechaNDate, date1);
                         fechaNancorrecta = (diferenciaDias > 18) ? txtFechaN.getDate() : null;
                    }
                    if (fechaNancorrecta != null){
                        huesped.setFechaDeNacimiento(fechaNancorrecta);
                    }else {
                        JOptionPane.showMessageDialog(null,"checa tu fecha bro");
                        throw new RuntimeException("error");
                    }

                    reserva.setHuesped(huesped);
                    huesped.setReserva(reserva);
                    huespedRepo.guardar(huesped);
                    transaction.commit();
                    Exito exito = new Exito();
                    exito.setVisible(true);
                }catch (Exception exception){
                    JOptionPane.showMessageDialog(null,"porfavor, revisa que tus datos sean correctos");
    ...
    private static Reserva getReserva() {
        ...
        reserva.setFechaCheckIn(txtFechaEntrada.getDate());
        reserva.setFechaCheckOut(txtFechaSalida.getDate());
